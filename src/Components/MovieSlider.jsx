  ...
  <SwiperSlide key={e.id} className="group relative">
    <div className="flex flex-col items-start justify-center">
      ...
      <div class="flex items-center">
          ...
          <p class="ms-2 text-sm font-bold text-gray-900 dark:text-white">{e.rating}</p>
          <span class="w-1 h-1 mx-1.5 bg-gray-500 rounded-full dark:bg-gray-400"></span>
          <a href="#" class="text-sm font-medium text-gray-900 underline hover:no-underline dark:text-white">{e.votes} Reviews</a>
      </div>
        </div>
      </div>


      <h3 className="font-semibold mt-5 text-lg m-auto">{e.title}</h3>
      <p className="text-gray-600 text-sm font-semibold m-auto">{e.language}</p>

      <div className="flex gap-4 mt-3 m-auto">
        <Link to={`/editForm/${e.id}`} className="p-2 rounded-md border border-gray-400 hover:bg-gray-200 transition">
